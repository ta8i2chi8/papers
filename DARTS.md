...
### メモ
* 探索プロセスを通じてアーキテクチャが変化するため、バッチ正規化にはグローバル移動平均ではなく、常にバッチ固有の統計値を使用します。（affine=Falseってことかな？）
* すべてのバッチ正規化における学習可能なアフィンパラメータは、候補となる演算の出力を再スケーリングしないように、探索プロセス中は無効にしています。